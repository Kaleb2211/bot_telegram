...
def pizza(update, context):
    ...
    if(update.message.text.upper().find("MANZANAS VERDES") > -1):
        update.message.reply_text("Prefiero comer pizza")
    else:
        if(update.message.text.upper().find("VERDES") > -1):
           update.message.reply_text("Tenemos muchas cosas verdes. Quieres manazanas???")
...
def main():
    ...
    updater = Updater("5217086135:AAHdtHZlRWAelxzioTd6ArcUozQguztZFhY", use_context=True)
